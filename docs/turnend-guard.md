...
- Codex registers a `Stop` hook in `.codex/hooks.json`, anchors the executable to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the shared guard.
  A firstmate-launched codex secondmate keeps this registration, but on codex 0.147.0 it is not a verified backstop.
  Codex 0.147.0 stops a launch on a `Hooks need review` dialog, so `bin/fm-spawn.sh` launches the codex secondmate template with `--dangerously-bypass-hook-trust`, which clears that dialog and leaves the hooks engine installed rather than switching it off: a marked secondmate home is a controlled primary whose hook source is this repository's own tracked `.codex/hooks.json`.
  That flag does NOT by itself mark the entry active - the in-session `/hooks` panel still reports it `Active 0 / Review 1`, and only persisted hook trust reaches `Active 1` - and whether it nonetheless executes at a turn boundary was not established, so do not rely on a codex secondmate's Stop guard until a credentialed turn-level probe closes that gap ([`verification/runtime-backends.md`](verification/runtime-backends.md) "Hooks review dialog").
  A pi secondmate, whose template loads `.pi/extensions/fm-primary-turnend-guard.ts` explicitly, remains the harness with a verified backstop.
  The codex crewmate and scout templates instead clear the same dialog with `-c 'features.hooks=false'`, because on 0.147.0 codex resolves project hooks from the main checkout rather than the cwd, so the file a crew pane is asked to trust is the captain's own PRIMARY-session hook set, which a worker must never trust or run.
  That is a deliberate trust decision rather than a gap in this guard, whose primary scope already excludes a linked task worktree; its live evidence and its cost are recorded in [`verification/runtime-backends.md`](verification/runtime-backends.md) "Hooks review dialog".
