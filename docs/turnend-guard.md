...
- Codex registers a `Stop` hook in `.codex/hooks.json`, anchors the executable to the hook process working directory, verifies a Firstmate-shaped hook-bearing root, and passes the original payload to the shared guard.
  A firstmate-launched codex secondmate keeps this registration in full.
  Codex 0.147.0 stops a launch on a `Hooks need review` dialog, so `bin/fm-spawn.sh` launches the codex secondmate template with `--dangerously-bypass-hook-trust`, which clears that dialog without disabling the hooks engine: a marked secondmate home is a controlled primary whose hook source is this repository's own tracked `.codex/hooks.json`, so it must retain its turn-end guard and its arm/cd `PreToolUse` seatbelts.
  The codex crewmate and scout templates instead clear the same dialog with `-c 'features.hooks=false'`, because a task worktree's branch may carry unvetted edits to that hook file and must never auto-trust it.
