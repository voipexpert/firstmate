...
Finalization never refuses an unscored task, because teardown finalizes every routed task and a refusal would strand its capacity: when no score was recorded, the terminal outcome keeps the legal `unknown`/`unknown`/`no` placeholder, is timestamped with the finalize-time clock so `elapsedSeconds` is a real duration rather than a fabricated zero, and carries `scored: false` so a never-scored record stays distinguishable from a deliberately recorded `unknown` score.
