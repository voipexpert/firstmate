...
Crewmate and scout dispatch can stay on the static crewmate harness resolved by `config/crew-harness`, use legacy version 1 dispatch profiles, or use version 2 subscription routing.
[`configuration.md`](configuration.md#crew-dispatch-profiles-configcrew-dispatchjson) owns both operator schemas, native account-lane configuration, fixed capacity controls, staged modes, and immediate rollback.
For version 2, `automatic-dispatch` classifies authorized work, checks the current catalogs and quota evidence, selects one normalized route per independent subtask, and uses the existing launcher and worktree lifecycle for admission.
The shell scripts validate policy and routing records and enforce capacity, circuit, privacy, and transactional lifecycle boundaries, but they do not parse task intent or expand task authority.
...
That keeps legacy spawn launch compatible across claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, and muse while version 2 remains limited to native Claude, native Codex, Pi, and pi-signed profiles.
