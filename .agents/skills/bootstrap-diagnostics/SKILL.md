...
- `CREW_DISPATCH: invalid config/crew-dispatch.json - invalid dispatch policy` - the optional dispatch profile file exists but failed low-cost bootstrap validation; stop profile-based dispatch and run `bin/fm-dispatch-policy.sh validate config/crew-dispatch.json` separately to obtain its one fixed reason category.
  Report that fixed category without quoting policy contents, parser output, or paths, and require correction rather than falling back around the invalid profile.
